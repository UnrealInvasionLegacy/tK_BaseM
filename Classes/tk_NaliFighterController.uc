class tk_NaliFighterController extends Bot;
...
function CheckIfShouldCrouch(vector StartPosition, vector TargetPosition, float probability)
{
    ...
    if ( !Pawn.bCanCrouch || (!Pawn.bIsCrouched && (FRand() > probability)) || (Skill < 3 * FRand()) || Monster(Pawn).RecommendSplashDamage() )
    {
    ...
    else if (Pawn.Physics == PHYS_Swimming)
    {
        if (Pawn.bCanFly)
	  {
        ...
        else
        {
            ...
            if (Pawn.bCanWalk && (Abs(Pawn.Acceleration.X) + Abs(Pawn.Acceleration.Y) > 0) && (Destination.Z >= Pawn.Location.Z) && Pawn.CheckWaterJump(jumpDir))
...
function FightEnemy(bool bCanCharge,float  EnemyStrength)
{
    ...
    Aggression = 1.5 * FRand() - 0.8 + 2 * AdjustedCombatStyle + FRand() * (Normal(Enemy.Velocity - Pawn.Velocity) Dot Normal(Enemy.Location - Pawn.Location));
    ...
    if ((Pawn.Physics == PHYS_Walking) || (Pawn.Physics == PHYS_Falling))
    {
        if (Pawn.Location.Z > Enemy.Location.Z + TACTICALHEIGHTADVANTAGE)
	  {
        ...
        else if (Pawn.Location.Z < Enemy.Location.Z - Pawn.CollisionHeight)
	  {
...
function float AdjustAimError(float aimerror, float TargetDist, bool bDefendMelee, bool bInstantProj, bool bLeadTargetNow)
{
    ...
    aimerror = aimerror * FMin(5,(12 - 11 * (Normal(Target.Location - Pawn.Location) Dot Normal((Target.Location + 1.2 * Target.Velocity) - (Pawn.Location + Pawn.Velocity)))));
...
function rotator AdjustAim(FireProperties FiredAmmunition, vector projStart, int aimerror)
{
    ...
    bClean = false;
    if ( FiredAmmunition.bTrySplash && (Pawn(Target) != None) && ((Skill >=4) || bDefendMelee) && (((Target.Physics == PHYS_Falling) && (Pawn.Location.Z + 80 >= Target.Location.Z))
...
function ReceiveWarning(Pawn shooter, float projSpeed, vector FireDir)
{
    ...
    if ( (Pawn.health <= 0) || (DodgeSkill < 4) || (Enemy == None) || !Monster(Pawn).bCanDodge || (Pawn.Physics == PHYS_Falling) || (Pawn.Physics == PHYS_Swimming)
...
ignores SeePlayer, HearNoise;
    ...
    function NotifyTakeHit(pawn InstigatedBy, vector HitLocation, int Damage, class<DamageType> damageType, vector Momentum)
    {
        ...
        if (StrafeFromDamage(damage, damageType, true))
            return;
        else if (bWasOnGround && (MoveTarget == Enemy) && (Pawn.Physics == PHYS_Falling))
        {
            pick = 1.0;
            if (bStrafeDir)
                pick = -1.0;
...
ignores SeePlayer, HearNoise;
    ...
    function bool NotifyHitWall(vector HitNormal, actor Wall)
    {
        ...
        if (bChangeDir || (FRand() < 0.5) || (((Enemy.Location - Pawn.Location) Dot HitNormal) < 0))
        {
            Focus = Enemy;
            WhatToDoNext(19);
        }
    ...
    function bool EngageDirection(vector StrafeDir, bool bForced)
    {
        ...
        Destination = MinDest + StrafeDir * (0.5 * MINSTRAFEDIST + FMin(VSize(Enemy.Location - Pawn.Location), MINSTRAFEDIST * (FRand() + FRand())));
        return true;
    }
...
Begin:
    ...
    FinishRotation();
    if (Monster(Pawn).HasRangedAttack() && (FRand() < 0.5) && (VSize(Enemy.Location - FocalPoint) < 150) && (Level.TimeSeconds - LastSeenTime < 4) && ClearShot(FocalPoint,true))
    {
        FireWeaponAt(Enemy);
    }
    else
